...
const Motorcycles = () => {
    const motorcycles = [
        {
            name: "Luna Racer",
            description: "The Luna Racer is a revival of the classic Cafe Racer style from the 70's. Neutral stance with the acceleration of a sport bike and nimble handling of street roadster.",
            performance: [
        ...
        {
            name: "Luna Scrambler",
            description: "The Tarform Luna comes in two different expressions - The Scrambler and The Racer. Sharing the same platform and powertrain, The Scrambler has a classic headlight, spoked wheels and knobby tires for all terrains.",
            performance: [
                "<strong>Top Speed</strong>: 120mph",
                "<strong>0 to 60</strong>: 3.8 Seconds",
                "<strong>Peak Torque</strong>: 120ft-lb (160Nm)",
    ...
    return (
        <>
            ...
            <h2 className='font-semibold text-4xl text-center relative before:content-[""] before:w-36 before:absolute before:-bottom-4 before:h-[2px] before:bg-white before:left-1/2 before:-translate-x-1/2'>
                <span className='font-normal text-base'>Checkout the Latest Bikes </span><br />Newest Bikes
            </h2>
            <div className="flex flex-col gap-6">
                {motorcycles.map((item, idx) => <AvailableMotorCycle key={item.name} item={item} isEven={idx % 2 == 0} />)}
            </div>
        </>
    )
}
